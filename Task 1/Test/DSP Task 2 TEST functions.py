...
def AddSignalSamplesAreEqual(userFirstSignal,userSecondSignal,Your_indices,Your_samples):
    if(userFirstSignal=='Signal1.txt' and userSecondSignal=='Signal2.txt'):
        file_name="testcases and testing functions/add.txt"  # write here the path of the add output file
...
def SubSignalSamplesAreEqual(userFirstSignal,userSecondSignal,Your_indices,Your_samples):
    if(userFirstSignal=='Signal1.txt' and userSecondSignal=='Signal2.txt'):
        file_name="testcases and testing functions/subtract.txt" # write here the path of the subtract output file
...
def MultiplySignalByConst(User_Const,Your_indices,Your_samples):
    if(User_Const==5):
        file_name="testcases and testing functions/mul5.txt"  # write here the path of the mul5 output file
...
def ShiftSignalByConst(Shift_value,Your_indices,Your_samples):
    if(Shift_value==3):  #x(n+k)
        file_name="testcases and testing functions/advance3.txt" # write here the path of delay3 output file
    elif(Shift_value==-3): #x(n-k)
        file_name="testcases and testing functions/delay3.txt" # write here the path of advance3 output file
    ...
    for i in range(len(Your_indices)):
        if(Your_indices[i]!=expected_indices[i]):
            print("Excepted Indices:", expected_indices)
...
def Folding(Your_indices,Your_samples):
    file_name = "testcases and testing functions/folding.txt"  # write here the path of the folding output file
    expected_indices,expected_samples=ReadSignalFile(file_name)      
    if (len(expected_samples)!=len(Your_samples)) and (len(expected_indices)!=len(Your_indices)):
